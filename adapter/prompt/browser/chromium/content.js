(() => {
  const SURFACE = 'browser_chromium';
  ...
  function isKnownAIHost() {
    return SUPPORT_HOSTS.some((h) => host === h || host.endsWith('.' + h));
  }

  function collectAISignals() {
    const parts = [
      document.title || '',
    ...
    for (const el of Array.from(document.querySelectorAll(signalSelectors.join(','))).slice(0, 80)) {
      parts.push(
    ...
    return parts.join(' ').replace(/\s+/g, ' ').trim().slice(0, 5000).toLowerCase();
  }

  function inferAISite() {
    if (isKnownAIHost()) {
      return {
        isAI: true,
        confidence: 1,
        source: 'known_domain',
        reason: 'known AI destination',
      };
    }

    const signals = collectAISignals();
    const reasons = [];
    let score = 0;

    if (/(^|[.-])(ai|gpt|llm|copilot|chatbot|assistant)([.-]|$)/.test(host) ||
        /(claude|gemini|perplexity|poe|mistral|groq|huggingface|phind|blackbox|writesonic|jasper|character\.ai|copy\.ai)/.test(host)) {
      score += 0.45;
      reasons.push('ai-like host');
    }

    if (/(ask anything|ask ai|chat with|ai assistant|chatbot|large language model|\bllm\b|prompt|generate|rewrite|summarize|model selector|choose model|deep research|copilot)/.test(signals)) {
      score += 0.35;
      reasons.push('ai-like page text');
    }

    const field = pickPromptField(document);
    if (field) {
      score += 0.2;
      reasons.push('visible prompt field');
    }

    if (field && findSendButton(field, field.closest('form'), field.closest('[data-testid*="composer"], [class*="composer"], [role="form"], form'))) {
      score += 0.15;
      reasons.push('send control');
    }

    const confidence = Math.min(0.99, Number(score.toFixed(2)));
    return {
      isAI: confidence >= 0.55,
      confidence,
      source: 'page_heuristic',
      reason: reasons.join(', ') || 'insufficient AI signals',
    };
  }
  ...
  function isSupportedRoute() {
    ...
    const detection = inferAISite();
    window.__themistoPromptCapture.lastDetection = detection;
  ...
  async function postJSON(path, payload, timeoutMs) {
    ...
    if (!resp) {
      throw new Error('no_response_from_background');
    }
    if (resp.error) {
  ...
  async function evaluatePrompt(promptText) {
    ...
    const payload = {
      ...
      destination_host: host,
      destination_path: window.location.pathname || '/',
      vendor: inferVendor() || (detection.isAI ? 'unknown_ai' : ''),
      service_category: detection.isAI ? 'ai_llm' : '',
      protocol: 'http',
      adapter_version: '1.0.1',
      metadata: {
        ai_site_detected: detection.isAI ? 'true' : 'false',
        ai_site_detection_source: detection.source,
        ai_site_detection_confidence: String(detection.confidence),
        ai_site_detection_reason: detection.reason,
